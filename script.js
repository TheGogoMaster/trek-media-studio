...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Create beat card function WITH THUMBNAIL SUPPORT (PROPERLY ADDED)
    function createBeatCard(beat) {
        const beatCard = document.createElement('div');
        beatCard.className = 'beat-card';
        
        // Thumbnail HTML - PROPERLY ADDED
        const thumbnailHTML = beat.thumbnail && beat.thumbnail.trim() !== '' ? 
            `<div class="beat-thumbnail">
                <img src="${beat.thumbnail}" alt="${beat.title}" onerror="this.onerror=null; this.src='https://images.unsplash.com/photo-1511379938547-c1f69419868d?w=400&h=225&fit=crop'">
            </div>` :
            `<div class="beat-thumbnail">
    ...
    // Add beat functionality - WITH THUMBNAIL SUPPORT
    if (addBeatBtn) {
        addBeatBtn.addEventListener('click', function() {
            ...
            // Create beat object WITH THUMBNAIL
            const newBeat = {
    ...
    // Function to save ALL current beats - WITH THUMBNAIL SUPPORT
...
---`
    ...
    // Admin Beat Management - WITH THUMBNAIL SUPPORT
    ...
    const adminBeatThumbnail = document.getElementById('adminBeatThumbnail'); // THUMBNAIL FIELD
    ...
    // Update beats list display - WITH THUMBNAIL SUPPORT
    function updateBeatList() {
        ...
        adminBeats.forEach((beat, index) => {
            ...
            const thumbnailHTML = beat.thumbnail ? 
                `<img src="${beat.thumbnail}" class="beat-item-thumb" alt="${beat.title}" style="width:50px;height:50px;object-fit:cover;border-radius:5px;">` : 
                '<div class="beat-item-thumb default" style="width:50px;height:50px;background:var(--accent-red);border-radius:5px;display:flex;align-items:center;justify-content:center;color:white;"><i class="fas fa-music"></i></div>';
            
            beatItem.innerHTML = `
                <div class="beat-item-info" style="display:flex;align-items:center;gap:15px;">
                    ${thumbnailHTML}
                    <div class="beat-details">
                        <h6 style="margin:0 0 5px 0;font-size:16px;">${beat.title}</h6>
                ...
                <button class="remove-beat" data-index="${index}" style="background:var(--accent-red);color:white;border:none;width:30px;height:30px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;">
    ...
    function updateImagePreview(previewElement, imageSrc, altText) {
        if (previewElement) {
            previewElement.innerHTML = `
                <img src="${imageSrc}" alt="${altText}" style="width:100%;height:100%;object-fit:cover;">
                <div class="preview-overlay" style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);color:white;padding:8px;text-align:center;font-size:0.9rem;">
                    <span>${altText}</span>
                </div>
            `;
        }
    ...
    // Add beat functionality - WITH THUMBNAIL SUPPORT (SYNC TO WEBSITE)
    if (adminAddBeatBtn) {
        adminAddBeatBtn.addEventListener('click', () => {
            ...
            // ADD TO ADMIN PANEL
            adminBeats.unshift(newBeat);
            saveAdminBeats();
            updateBeatList();
            updateBeatCount();
            
            // ALSO ADD TO WEBSITE BEATS
    ...
    // Clear all beats - WORKS FOR BOTH ADMIN AND WEBSITE
    if (clearAllBeatsBtn) {
